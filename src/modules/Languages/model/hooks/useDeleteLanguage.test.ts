...
describe("useDeleteLanguage", () => {
  ...
  it("returns mutation function", () => {
    ...
    expect(result.current.deleteLanguage).toBe(deleteLanguageMock);
  });
  ...
  it("calls useMutationNotification with translated messages", () => {
    ...
    expect(useMutationNotification).toHaveBeenCalledWith({
      successMessage: "languageDeletedSuccessfully",
      errorMessage: "failedToDeleteLanguage",
    });
  });

  it("calls useMutation with correct arguments", () => {
    renderHook(() => useDeleteLanguage());
    expect(useMutation).toHaveBeenCalledWith("DELETE_LANGUAGE", {
      ...notifications,
